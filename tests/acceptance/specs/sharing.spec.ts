...
test("@full editor sharing, single-use invites, and reader admission", async ({
  browser,
}) => {
  const webUrl =
    process.env.KAMORI_ACCEPTANCE_WEB_URL ?? "http://127.0.0.1:14173";
  ...
  try {
    ...
    await createTask(owner, "Owner shared task");
    ...
    await owner
      .getByPlaceholder("Optional encrypted note for recipient")
      .fill("editor acceptance note");
    await owner.getByRole("button", { name: "Generate Invite Code" }).click();
    const editorCode = (
      await owner.getByText(inviteCodePattern, { exact: true }).textContent()
    )?.trim();
    ...
    await expect(
      editor.getByText("editor acceptance note", { exact: true }),
    ).toBeVisible();
    await syncNow(editor);
    await openAppSection(editor, "Tasks");
    await expect(
      editor.getByText("Owner shared task", { exact: true }),
    ).toBeVisible();

    await createTask(editor, "Editor shared task");
    await syncNow(owner);
    await openAppSection(owner, "Tasks");
    await expect(
      owner.getByText("Editor shared task", { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      reader.getByText("Owner shared task", { exact: true }),
    ).toBeVisible();
    await expect(
      reader.getByRole("button", { name: "New task", exact: true }),
    ).toBeDisabled();
    await expect(
      reader.getByText("Reader must not write", { exact: true }),
    ).toHaveCount(0);
  } finally {
    await Promise.all([
      ownerContext.close(),
