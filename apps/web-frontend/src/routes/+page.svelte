...
<header class="site-header">
    <a class="brand" href="/" aria-label="Kamori home">
        <BrandMark size={38} />
        <span>KAMORI</span>
    </a>
    ...
    <section class="numbered-section" id="apps">
        ...
        <div class="app-ledger">
            <article>
                ...
                <h3>{c.webTitle}</h3>
                <p>{c.webBody}</p>
                <a href="/app?start=signup">{c.webAction} →</a>
            </article>
...
<style>
    ...
    .site-header { width: min(1320px, calc(100% - 3rem)); margin: 0 auto; min-height: 84px; display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; border-bottom: 1px solid var(--rule); }
    ...
    nav { display: flex; gap: clamp(1rem, 2.3vw, 2.4rem); }
    nav a, .text-link { color: var(--ink); font-size: .82rem; text-decoration: none; }
    ...
    .numbered-section { display: grid; grid-template-columns: minmax(130px, .3fr) minmax(0, 1.7fr); column-gap: clamp(2rem, 7vw, 7rem); padding: 7rem 0; border-bottom: 1px solid var(--rule); scroll-margin-top: 1rem; }
    ...
    @media (max-width: 680px) {
        .site-header, main, footer { width: min(100% - 2rem, 1320px); }
